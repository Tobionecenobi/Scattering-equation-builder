...
public:
    ...
    //returns the absolute reference points of the structure
    AbsoluteReferencePointSet getAbsRef(){
        return StructRefPoints;
    }

    void clearAbsRefPoints(){
        StructRefPoints.clear();
    }

    void insertAbsRefPoints( const AbsRefPoint &a) {
        StructRefPoints.insert( a );
    }
    
    void printAbsRefSet(){
        for(auto i = StructRefPoints.begin(); i != StructRefPoints.end(); i++){
